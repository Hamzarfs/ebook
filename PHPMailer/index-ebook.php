...
try {
    ...
    if (!is_array($data) || !isset($data['name'], $data['email'], $data['message'], $data['subject'])) {
        throw new Exception('Invalid or missing form data.');
    }

    // Sanitize and extract form fields
    $name       = htmlspecialchars($data['name'] ?? ''); // Handle if 'page_title' is not present
    ...
    $mail->Username   = 'dev@amzbookpublishing.net'; // Your email address
    $mail->Password   = 'Amazonebook@123';             // Your email password
    $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS; // Encryption method
    $mail->Port       = 587;                            // TCP port to connect to

    // Send email to admin
    $mail->setFrom('dev@amzbookpublishing.net', 'amzbookpublishing');
    ...
    $mail->Subject = 'Confirmation of Form Submission: amzbookpublishing';
